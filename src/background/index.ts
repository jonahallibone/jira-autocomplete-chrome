...
class Background {
   ...
   */
  init = () => {
    console.info('[===== Loaded Background Scripts =====]');
   ...
   */
  onInstalled = () => {
    console.info('[===== Installed Extension!] =====');
  };
  ...
  onMessage = async (message: EXTMessage, sender: Runtime.MessageSender) => {
    try {
      console.info('[===== Received message =====]', message, sender);
  ...
  onMessageFromExtension = (msg: EXTMessage) => {
    console.info('[===== Message from Long Live Connection =====]', msg);
  };
  ...
  onUpdatedTab = (
    ...
    console.info('[===== Tab Created =====]', tabId, changeInfo, tab);
  };
